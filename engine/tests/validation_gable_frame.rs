...
fn make_gable_frame(
    ...
    if gravity_load.abs() > 1e-20 {
        ...
        loads.push(SolverLoad::Nodal(SolverNodalLoad {
            node_id: 4, fx: 0.0, fy: gravity_load, mz: 0.0,
        }));
    }
...
fn validation_gable_symmetric_gravity() {
    ...
    assert_close(r5.ry, -total_gravity / 2.0, 0.01,
    ...
    assert_close(r1.mz.abs(), r5.mz.abs(), 0.01,
...
fn validation_gable_lateral_wind() {
    ...
    assert!(r1.mz.abs() > 0.1, "Gable wind: base moment at 1 exists");
    assert!(r5.mz.abs() > 0.1, "Gable wind: base moment at 5 exists");
}
...
fn validation_gable_a_frame() {
    ...
    // ΣFy = 0
    assert_close(r1.ry + r5.ry, -p, 0.01, "A-frame: ΣFy = P");

    // ΣFx = 0
    assert_close(r1.rx + r5.rx, 0.0, 0.01, "A-frame: ΣFx = 0");

    // By symmetry: Ry1 = Ry5 = P/2
    assert_close(r1.ry, -p / 2.0, 0.01, "A-frame: Ry1 = P/2");
...
fn validation_gable_vs_flat() {
    ...
    let ry_flat: f64 = res_flat.reactions.iter().map(|r| r.ry).sum();
    ...
    let ry_gable: f64 = res_gable.reactions.iter().map(|r| r.ry).sum();
    ...
    assert!(d_ridge.uy.abs() > 1e-6, "Gable: ridge deflects vertically");
}
...
fn validation_gable_asymmetric() {
    ...
    // Global equilibrium
    assert_close(r1.ry + r5.ry, -p, 0.01, "Asymmetric gable: ΣRy = P");
...
fn validation_gable_pinned_roller() {
    ...
    // ΣFy = 0
    assert_close(r1.ry + r5.ry, -p, 0.01, "Gable roller: ΣRy = P");
