...
fn test_envelope_3d_json_field_name() {
    ...
    let env = FullEnvelope3D {
        ...
        max_abs_results_3d: AnalysisResults3D {
            displacements: vec![],
            reactions: vec![],
            element_forces: vec![], plate_stresses: vec![], quad_stresses: vec![], quad_nodal_stresses: vec![], constraint_forces: vec![], diagnostics: vec![], solver_diagnostics: vec![], timings: None, structured_diagnostics: vec![], equilibrium: None, result_summary: None, solver_run_meta: None },
    };
...
fn test_parity_3d_combination_superposition() {
    ...
    let make_3d_cantilever = |loads: Vec<SolverLoad3D>| -> SolverInput3D {
        ...
        SolverInput3D { solver_options: None, nodes, materials, sections, elements, supports, loads, constraints: vec![], left_hand: None, plates: HashMap::new(), quads: HashMap::new(), quad9s: HashMap::new(), solid_shells: HashMap::new(), curved_shells: HashMap::new(), curved_beams: vec![] , connectors: HashMap::new() }
    };
