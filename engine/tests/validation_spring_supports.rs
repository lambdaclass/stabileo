...
fn validation_spring_rotational() {
    ...
    // Cantilever (fixed)
    let input_fixed = helpers::make_beam(n, l, E, A, IZ, "fixed", None,
        vec![SolverLoad::Nodal(SolverNodalLoad {
...
fn validation_spring_very_stiff() {
    ...
    let mid = n / 2 + 1;
    let input_rigid = helpers::make_beam(n, l, E, A, IZ, "pinned", Some("rollerX"),
        vec![SolverLoad::Nodal(SolverNodalLoad {
...
fn validation_spring_very_soft() {
    ...
    // Cantilever (free end)
    let input_free = helpers::make_beam(n, l, E, A, IZ, "fixed", None,
        vec![SolverLoad::Nodal(SolverNodalLoad {
            node_id: n + 1, fx: 0.0, fy: -p, mz: 0.0,
        })]);
