...
fn validation_pc_ext_4_bearing_pad() {
    ...
    helpers::assert_close(s, 4.63, 0.01, "Shape factor S");
...
fn validation_pc_ext_5_shear_key() {
    ...
    helpers::assert_close(v_u_stress, 1.0, 0.01, "Unit shear stress");
...
fn validation_pc_ext_6_erection_bracing() {
    ...
    helpers::assert_close(e_sweep, 25.0, 0.01, "Sweep imperfection");
    ...
    helpers::assert_close(brace_force, 7.2, 0.01, "Bracing force");
...
fn validation_pc_ext_7_connection_ductility() {
    ...
    helpers::assert_close(delta_y, 0.25, 0.01, "Yield displacement");

    // Ultimate displacement (assume 5% elongation at fracture)
    let eps_u: f64 = 0.05;
    let delta_u: f64 = eps_u * l_eff; // mm = 10.0 mm

    helpers::assert_close(delta_u, 10.0, 0.01, "Ultimate displacement");
...
fn validation_pc_ext_8_camber_prediction() {
    ...
    helpers::assert_close(delta_straight, delta_up, 0.01, "Straight vs parabolic camber");
