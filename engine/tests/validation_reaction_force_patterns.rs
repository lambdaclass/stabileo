...
fn validation_ss_beam_asymmetric_point_load() {
    ...
    assert_close(r_a.ry, r_a_exact, 0.02, "SS asymmetric: R_A = P*(L-a)/L");

    // R_B = P*a/L = P/3 = 12
    let r_b_exact = p * a_frac;
    assert_close(r_b.ry, r_b_exact, 0.02, "SS asymmetric: R_B = P*a/L");

    // Sum of reactions = P
    assert_close(r_a.ry + r_b.ry, p, 0.02, "SS asymmetric: R_A + R_B = P");
}
...
fn validation_cantilever_two_point_loads() {
    ...
    assert_close(r.ry, p1 + p2, 0.02, "Cantilever 2P: Ry = P1 + P2");
    ...
    assert_close(r.mz.abs(), m_expected, 0.02, "Cantilever 2P: M = P1*L/3 + P2*2L/3");
...
fn validation_fixed_fixed_asymmetric_load() {
    ...
    assert_close(r_a.ry, r_a_exact, 0.02, "Fixed-fixed asym: R_A = Pb^2(3a+b)/L^3");

    // R_B = P*a^2*(a+3b)/L^3 = P*a^2*(L+2b)/L^3
    let r_b_exact = p * a * a * (a + 3.0 * b) / l.powi(3);
    assert_close(r_b.ry, r_b_exact, 0.02, "Fixed-fixed asym: R_B = Pa^2(a+3b)/L^3");

    // Equilibrium check
    assert_close(r_a.ry + r_b.ry, p, 0.02, "Fixed-fixed asym: R_A + R_B = P");

    // R_A > R_B since load is closer to left
    assert!(r_a.ry > r_b.ry, "Fixed-fixed asym: R_A > R_B (load closer to A)");
}
...
fn validation_propped_cantilever_center_load() {
    ...
    // R_B = 5P/16
    assert_close(r_b.ry, 5.0 * p / 16.0, 0.02, "Propped center: R_B = 5P/16");

    // R_A = 11P/16
    assert_close(r_a.ry, 11.0 * p / 16.0, 0.02, "Propped center: R_A = 11P/16");

    // M_A = 3PL/16
    assert_close(r_a.mz.abs(), 3.0 * p * l / 16.0, 0.02, "Propped center: M_A = 3PL/16");

    // Equilibrium: R_A + R_B = P
    assert_close(r_a.ry + r_b.ry, p, 0.02, "Propped center: R_A + R_B = P");
}
...
fn validation_two_span_one_loaded_uplift() {
    ...
    let sum_ry = r_a.ry + r_b.ry + r_c.ry;
    ...
    assert!(r_c.ry < 0.0,
        "2-span loaded: R_C < 0 (uplift at far end), got R_C = {:.4}", r_c.ry);

    // Support B carries the most since it is adjacent to the loaded span
    assert!(r_b.ry > r_a.ry,
        "2-span loaded: R_B > R_A, got R_B={:.4}, R_A={:.4}", r_b.ry, r_a.ry);
}
...
fn validation_roller_zero_horizontal_reaction() {
    ...
    assert_close(r_pin.mz, 0.0, 0.02, "Roller: Mz_pinned = 0");
    assert_close(r_roller.mz, 0.0, 0.02, "Roller: Mz_roller = 0");

    // Vertical reactions should each be P/2 by symmetry
    assert_close(r_pin.ry, p / 2.0, 0.02, "Roller: Ry_pinned = P/2");
    assert_close(r_roller.ry, p / 2.0, 0.02, "Roller: Ry_roller = P/2");
}
...
fn validation_fixed_support_moment_reaction() {
    ...
    assert!(r_a.mz.abs() > 1.0,
        "Fixed moment: M_A nonzero, got {:.4}", r_a.mz);
    assert!(r_b.mz.abs() > 1.0,
        "Fixed moment: M_B nonzero, got {:.4}", r_b.mz);

    // Moment reactions should have opposite signs
    // (one end hogging, other end sagging in the global sense)
    assert!(r_a.mz * r_b.mz < 0.0,
    ...
    assert_close(r_b.mz.abs(), m_b_exact, 0.05,
...
fn validation_three_support_interior_gt_end() {
    ...
    assert_close(r_a.ry, r_end_exact, 0.05,
        "3-support: R_A = 3qL/8");
    assert_close(r_c.ry, r_end_exact, 0.05,
        "3-support: R_C = 3qL/8");

    // Interior reaction: R_B = 10qL/8 = 5qL/4
    let r_int_exact = 10.0 * q.abs() * span / 8.0;
    assert_close(r_b.ry, r_int_exact, 0.05,
        "3-support: R_B = 10qL/8");

    // Interior reaction > end reactions
    assert!(r_b.ry > r_a.ry,
        "3-support: R_B ({:.4}) > R_A ({:.4})", r_b.ry, r_a.ry);
    assert!(r_b.ry > r_c.ry,
        "3-support: R_B ({:.4}) > R_C ({:.4})", r_b.ry, r_c.ry);
