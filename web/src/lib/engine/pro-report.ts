...
`);
  ...
  if (showSection('verification') && verifications.length > 0) {
    ...
    for (const group of groups) {
      ...
      html.push(`<h4>${escHtml(tr('report.flexureCheck'))}</h4>`);
      ...
      html.push(`<h4>${escHtml(tr('report.shearCheck'))}</h4>`);
      html.push(`<div class="step-block">`);
      ...
      if (v.column) {
        html.push(`<h4>${escHtml(tr('report.compressionCheck'))}</h4>`);
        html.push(`<div class="step-block">`);
      ...
      if (v.torsion) {
        html.push(`<h4>${escHtml(tr('report.torsionCheck'))}${v.torsion.neglect ? ` (${escHtml(tr('report.torsionNeglect'))})` : ''}</h4>`);
        html.push(`<div class="step-block">`);
      ...
      if (v.biaxial) {
        html.push(`<h4>${escHtml(tr('report.biaxialCheck'))}</h4>`);
        html.push(`<div class="step-block">`);
