...
export function chooseBarsForCount(AstCm2: number, count: number): BarChoice {
  const per = AstCm2 / Math.max(count, 1);
  const bar = REBAR_DB.find((r) => r.diameter >= 10 && r.area >= per)
    ?? REBAR_DB[REBAR_DB.length - 1];
  return {
    count,
    diameter: bar.diameter,
    areaCm2: count * bar.area,
    label: `${count} Ø${bar.diameter}`,
    fitsInOneLayer: null,
    clearSpacingMm: null,
  };
}
