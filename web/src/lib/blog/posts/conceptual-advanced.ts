 ...
 *
 * What none of the tools we reviewed does is the conjunction: explain
 * kinematic analysis, produce a bar schedule, and develop section analysis.
 * None of them does two, and none approaches any of the three conceptually.
 * The review was not exhaustive, so the post states this as what we found
 * ("we found no…", "among the tools we reviewed…"), never as "there is none".
 * That is the claim, and naming what each one is good at is what makes it
...
export const conceptualAdvanced: Post = {
  ...
  i18n: {
    es: {
      ...
      excerpt:
        'Hay software estructural gratuito y muy bueno: modela, resuelve y muestra resultados. Lo que no encontramos es uno que encare las funciones avanzadas del lado conceptual.',
      blocks: [
        ...
        { k: 'p', t: 'Los calculadores de torsión que se encuentran dando vueltas ilustran el punto al revés: vos elegís el tipo de sección (maciza, tubo cerrado, rectángulo) y el programa aplica la fórmula asociada. Ninguno de los que vimos te dice que elegiste mal, ni qué habría dado la otra teoría, ni de qué lado del error quedás.' },
        ...
        { k: 'note', t: 'Una aclaración que corresponde: nada de esto dice que seamos mejores. Ftool modela más rápido y con menos fricción que nosotros. MASTAN2 tiene detrás un libro que nosotros no escribimos. Los cálculos a mano de SkyCiv están mejor presentados que varios de nuestros paneles. Y sectionproperties resuelve alabeo por elementos finitos, que nosotros aproximamos. Lo que sostenemos es una cosa sola y bastante específica: entre las herramientas gratuitas que revisamos, ninguna encara las funciones avanzadas del lado conceptual. Si conocés una herramienta gratuita que explique el análisis cinemático, arme un despiece o desarrolle el análisis de sección, escribinos y corregimos esta nota.' },
      ],
    },
    en: {
      title: 'What free software computes, and what it does not explain',
      excerpt:
        'There is free structural software, and it is good: it models, it solves, it shows results. What we could not find is one that approaches the advanced tools from the conceptual side.',
      blocks: [
        { k: 'p', t: 'It is worth starting with what does exist: there is plenty of it, and it is good. Anyone teaching structures today has first-rate free tools available, and several of them do their job better than we do ours.' },

        { k: 'h', t: 'What the free tools actually cover' },
        {
          k: 'table',
          caption: 'Free structural analysis tools, and what they show beyond the result.',
          head: ['Tool', 'Free', 'What it does very well', 'Conceptual side'],
          rows: [
            ['Ftool', 'Yes', 'Modelling plane frames and reading diagrams and influence lines, with a fluency few match', 'Shows the behaviour, not the reasoning behind it'],
            ...
            ['SkyCiv', 'Free tier', 'Step-by-step hand calculations: free bodies, equilibrium equations before substitution, moment distribution for indeterminate beams', 'Numbers plugged into formulas, for basic statics'],
        ...
        { k: 'p', t: 'Of that list, SkyCiv comes closest to what we usually call "showing the working", and it should be said plainly: its hand calculations are good. It draws the free body, writes ΣM = 0 before any number goes in, and solves by moment distribution when the beam is indeterminate. Anyone who claims that no free program shows its steps can be proved wrong with SkyCiv in about a minute.' },

        { k: 'h', t: 'Showing the steps is not enough if the program does not tell you when the formula stops applying' },
        ...
        { k: 'p', t: 'The torsion calculators you find online illustrate the point in reverse: you pick the section type (solid, closed tube, rectangle) and the program applies the matching formula. None of the ones we saw tells you that you picked wrong, what the other theory would have given, or which side of the error you are on.' },
        { k: 'quote', t: 'A program that only computes will never warn you that you are computing the wrong thing.' },

        { k: 'h', t: 'Three features with no equivalent we could find' },
        {
          k: 'ul',
          items: [
            'Kinematic analysis, explained. We looked and found no free tool that does this: what turns up is lecture material in PDF. The "educational software for kinematic analysis" that we did find is for machine mechanisms (linkages and cranks), not for classifying structures.',
            'Rebar detailing. The only free option we found is AutoRebar, and it is an AutoCAD add-on, so using the free part requires an expensive licence. It also works from your drawing: you place the bars, it builds the schedule. Everything else is a seven-day trial or commercial software. We found no open-source option.',
        ...
        { k: 'p', t: 'The formula for the degree of indeterminacy gives g = 3·m + r − 3·n = 3×2 + 3 − 3×3 = 0. Zero means statically determinate, and any program that stops at the formula will report exactly that. But nothing holds the beam horizontally: it is a mechanism, and it cannot be solved.' },
        {
          k: 'table',
        ...
        { k: 'p', t: 'Nothing has to be installed or licensed, and the department does not have to buy anything. It opens in a browser, and the model is shared as a link.' },

        { k: 'note', t: 'To be clear: none of this means we are better. Ftool models faster and with less friction than we do. MASTAN2 has a textbook behind it that we did not write. SkyCiv\'s hand calculations are better presented than several of our panels. And sectionproperties solves warping by finite elements where we approximate. What we claim is narrower and quite specific: among the free tools we reviewed, none approaches the advanced tools from the conceptual side. If you know a free tool that explains kinematic analysis, produces a bar schedule, or works through section analysis, write to us and we will correct this post.' },
      ],
    },
    pt: {
      title: 'O que o software gratuito calcula e o que ele não explica',
      excerpt:
        'Existe software estrutural gratuito e muito bom: modela, resolve e mostra resultados. O que não encontramos foi um que aborde as funções avançadas pelo lado conceitual.',
      blocks: [
        { k: 'p', t: 'Vale começar pelo que já existe, que é bastante coisa e é bom. Quem ensina estruturas hoje tem ferramentas gratuitas de primeira linha, e várias delas fazem o que se propõem a fazer melhor do que nós.' },

        { k: 'h', t: 'O que as ferramentas gratuitas realmente cobrem' },
        {
          k: 'table',
          ...
          rows: [
            ...
            ['MASTAN2', 'Sim (roda no MATLAB ou no seu runtime)', 'Análise matricial linear e não linear, ligada ao livro de McGuire, Gallagher e Ziemian', 'A teoria está no livro; o programa roda a análise'],
            ['SkyCiv', 'Plano gratuito', 'Cálculos manuais passo a passo: diagramas de corpo livre, equações de equilíbrio antes da substituição, método de Cross para vigas hiperestáticas', 'Números substituídos em fórmulas, para estática básica'],
        ...
        { k: 'p', t: 'As calculadoras de torção que circulam por aí ilustram o ponto ao contrário: você escolhe o tipo de seção (maciça, tubo fechado, retângulo) e o programa aplica a fórmula correspondente. Nenhuma das que vimos avisa que você escolheu errado, nem o que a outra teoria teria dado, nem de que lado do erro você ficou.' },
        { k: 'quote', t: 'Um programa que só calcula nunca vai avisar que você está calculando a coisa errada.' },

        { k: 'h', t: 'Três funções para as quais não encontramos equivalente' },
        {
          k: 'ul',
          items: [
        ...
        { k: 'p', t: 'A fórmula do grau de hiperestaticidade dá g = 3·m + r − 3·n = 3×2 + 3 − 3×3 = 0. Zero significa isostática, e qualquer programa que pare na fórmula vai informar exatamente isso. Mas nada segura a viga na horizontal: é um mecanismo, e não tem solução.' },
        {
          k: 'table',
        ...
        { k: 'p', t: 'O app não se limita a informar o zero e lavar as mãos. Ele contradiz esse resultado no mesmo passo 2 em que o calculou: "A fórmula dá g = 0 (condição necessária para isostática), mas NÃO suficiente". O passo 3 monta a matriz de rigidez global, 6×6 aqui, e marca "Detectado 1 modo de mecanismo" apesar de g = 0 ≥ 0. Aponta o nó 3 e o seu deslocamento horizontal, explica que a equação do passo 2 conta restrições sem olhar onde elas estão e fecha com "✗ Mecanismo — não pode ser resolvida".' },
        {
          k: 'embed',
          query: 'example=hidden-mechanism&kin=1',
          label: 'A viga sobre três apoios verticais, com o painel de análise cinemática aberto. Percorra os quatro passos: o segundo dá g = 0 e o terceiro encontra o mecanismo que esse zero esconde. Troque um apoio por um fixo e rode de novo: o grau passa a 1 e o mecanismo desaparece.',
        },

        { k: 'h', t: 'Por que isso pode interessar a um departamento' },
        { k: 'p', t: 'Um aluno que aprende a fórmula do grau e nunca vê a condição suficiente falhar sai com uma regra incompleta sem saber. É difícil mostrar isso no quadro, porque o exemplo que a derruba parece um erro de digitação. Aqui aparece numa tela só, com o número e a contradição lado a lado.' },
        { k: 'p', t: 'E não é preciso instalar nada, ter licença ou pedir que a faculdade compre alguma coisa. Abre no navegador, e o modelo é compartilhado por link.' },

        { k: 'note', t: 'Um esclarecimento necessário: nada disso quer dizer que somos melhores. O Ftool modela mais rápido e com menos atrito que nós. O MASTAN2 tem por trás um livro que nós não escrevemos. Os cálculos manuais do SkyCiv são mais bem apresentados que vários dos nossos painéis. E o sectionproperties resolve empenamento por elementos finitos, onde nós fazemos uma aproximação. O que defendemos é uma coisa só, e bem específica: entre as ferramentas gratuitas que analisamos, nenhuma aborda as funções avançadas pelo lado conceitual. Se você conhece uma ferramenta gratuita que explique análise cinemática, gere um detalhamento de armaduras ou desenvolva a análise de seção, escreva para nós e corrigimos esta nota.' },
      ],
    },
  },
};
