...
export function modelToCode(snap: ModelSnapshot): string {
  ...
  const settings = ['massSource', 'resultScopes', 'combinationRules', 'views', 'grid', 'geotechnical', 'footingMatPreferences', 'codeSettings', 'regulations'].filter((k) => s[k] !== undefined);
  if (settings.length) {
...
export function codeToModel(text: string): ParseResult {
  ...
  lines.forEach((raw, idx) => {
    ...
    switch (kw!.text) {
      ...
      case 'massSource': case 'resultScopes': case 'combinationRules': case 'views': case 'grid': case 'geotechnical': case 'footingMatPreferences': case 'codeSettings': case 'regulations': {
