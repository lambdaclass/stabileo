<script lang="ts">
  ...
  import {
    ...
    generateBeamLongSectionSvg,
    beamScheduleToCSV,
    type BeamMark,
  } from '../../lib/engine/beam-schedule';
  ...
  const marks = $derived(groupBeamsByMark(verifications, elementLengths));
  const rows = $derived(buildBeamScheduleRows(marks));
  ...
  function exportCSV() {
    const csv = beamScheduleToCSV(marks);
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'beam-schedule.csv';
    a.click();
...
<div class="beam-schedule">
  {#if marks.length === 0}
    <div class="empty-msg">{t('pro.beamSchedNoBeams')}</div>
  {:else}
    <div class="schedule-header">
      <span class="schedule-title">{t('pro.beamSchedTitle')}</span>
      <span class="schedule-count">{marks.length} {t('pro.beamSchedMarks')} · {marks.reduce((s, m) => s + m.elements.length, 0)} {t('pro.beamSchedElements')}</span>
      <button class="csv-btn" onclick={exportCSV}>{t('pro.beamSchedExportCsv')}</button>
    </div>

    <table class="schedule-table">
      <thead>
        <tr>
          <th>{t('pro.beamSchedMark')}</th>
          <th>{t('pro.colSchedDims')}</th>
          <th>f'c</th>
          <th>{t('pro.beamSchedBotBars')}</th>
          <th>{t('pro.beamSchedTopBars')}</th>
      ...
      <tbody>
        {#each rows as row, i}
          ...
          <tr class="schedule-row" class:expanded={expandedMark === mark.mark} onclick={() => toggleMark(mark.mark)}>
            ...
            <td>{row.fc}</td>
            <td>{row.bottomBars}</td>
            <td>{row.topBars}</td>
            <td>{row.stirrupConfig}</td>
                ...
                <div class="mark-detail">
                  <div class="svgs">
                    <div class="svg-box">
                      ...
                      {@html generateBeamCrossSectionSvg(mark)}
                    </div>
                    <div class="svg-box">
                      <span class="svg-label">{t('pro.beamSchedLongSection')}</span>
                      {@html generateBeamLongSectionSvg({
                        mark,
                        span: mark.elements[0]?.span ?? 5.0,
                      })}
                    </div>
                  </div>
                  <div class="elements-list">
                    <span class="list-label">{t('pro.beamSchedElements')}:</span>
