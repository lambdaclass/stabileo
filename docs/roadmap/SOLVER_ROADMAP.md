...
**Done when:**
- ~~PCG converges on representative shell models~~ — DONE (2026-09-22): PCG-Jacobi converges on MITC4 simply-supported plates up to 50×50 (15.4k free DOFs) at tol 1e-8 with verified true residual ≤ 1e-6, 3.5–6.7× faster than direct end-to-end (11× on the solve phase at 50×50); frames converge with IC(0)/shifted-IC. The blocker was never the preconditioner family but the fixed stagnation safeguard aborting legitimate slow convergence (measured plateaus ≈ 3.2·√n); IC variants on thin shells documented as non-viable (see above). AMG remains relevant only for ≥100k-DOF shells
...
- ~~Hybrid solver automatically selects iterative above measured crossover point~~ — DONE (2026-09-21): `ITERATIVE_THRESHOLD = 1_000` from `pcg_bench` measurements (Apple M3)
