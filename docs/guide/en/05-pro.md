...
This chapter covers the **Model** and **Analyse** tabs.
...
- **An example:** **Project → New model → Examples**. There are sixteen, grouped into buildings,
  industrial, foundations, long-span structures, energy and offshore, and large showcase models.
...
its own **Draw** button (node, member, plate…); pressing it again goes back to selecting.
...
**Members.** A table with start and end node, material, section and the restraint at each end.
Also:
...
- Right-clicking a member: edit it (material, section, releases per degree of freedom), divide it
  into N parts, or mirror it.
...
- **Mesh generator:** from four corners, it generates a mesh of quadrilaterals by target size or
  by number of divisions. It can split the boundary beams so they share the edge nodes.
...
**Supports.** Fixed, pinned, rollers in each plane (XZ, XY, YZ), spring (with a stiffness for
each degree of freedom) and **custom**, where you tick one by one which displacements and
...
- **Load cases:** each case with its type (D, L, Lr, W, E, S). **Self-weight** is **on by
  default** in PRO and is computed for members and plates.
...
It first shows the load plan for review, and then applies it. Area loads become line loads on the
...
- **Truss:** trapezoidal, parallel-chord, Pratt, arched, or a solid-web portal, with several web
  patterns.
- **Lattice column.**
- **Shed:** span, frame spacing, number of frames, lattice or solid-web columns, purlins and
  bracing.

The generator **replaces the current model** (a single undo brings it back).

## Importing models

From **Project**:

- **Excel spreadsheet.** The same format as in Basic, with extra sheets for plates, constraints
  and loads. It is the most convenient way to load a large model prepared in another tool.
- **DXF plan (AutoCAD).** A four-step wizard:
  1. the file and its units;
  2. what each drawing layer represents: grid, columns, beams, walls, slabs, openings;
...
- **IFC (BIM).** Imports the model's beams, columns and members with their geometry.

## Before solving: diagnostics

PRO checks the model as you build it. The **Diagnostics** panel separates:
...
- **Colour map** of moment, shear, axial force, utilisation σ/fy, von Mises, or **slab and wall
  contours**.
...
- View by **case**, **combination** or **envelope**.
...
- **P-Delta**, **modal**, **spectral** (with CQC or SRSS combination, by seismic zone and soil type)
  and **buckling**.
...
- **3D influence lines**, **multi-case solver** and **section analyser**.
...
- **Triangular plates:** the **DKT** element for bending, combined with a constant-strain triangle
  for the membrane.
...
- **Modal and spectral analysis** take only the members into account: they do not yet include
  plates or constraints. Advanced analyses in general need at least one member in the model and
  ignore member and plate offsets.
- **Surface loads** apply only to quadrilateral plates, always vertical (global −Z), split equally
  among the four nodes.
- **Thermal loads on slabs** are not yet applied in the analysis.
- **Automatic load generation** does not load plates, and seismic generation is not yet enabled by
  default.
...
- **IFC** import brings in beams, columns and members, with a single material and a single section
  for all of them; it does not bring supports, loads, slabs or walls.
