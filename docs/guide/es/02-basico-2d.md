...
- **Selección** elige qué toma un clic o un arrastre: nodos, barras, o ambos. Arrastrar de
  izquierda a derecha toma lo que queda entero adentro del rectángulo; de derecha a izquierda,
  todo lo que toca. También se puede seleccionar **por id**, con listas y rangos como `3, 7-10`.
...
**Nodo** (tecla `N`) coloca un nodo con cada clic. Si el nodo cae sobre una barra, el programa
puede partirla en dos automáticamente (se activa en **Ajustes**).
...
- **Deslizadera X / Z.** Libera un desplazamiento relativo en una dirección, en ejes globales o
  locales.
...
- **Rígida (pórtico):** transmite axial, corte y momento. Es la barra de las vigas y columnas.
- **Articulada (reticulado):** sólo axial. Es la barra de los reticulados.

Las articulaciones también se pueden poner o sacar desde la tabla de barras, extremo por extremo
(columnas **Art. I** y **Art. J**).

> **Qué es una articulación, internamente.** Una barra con un extremo articulado no se resuelve
> con una matriz distinta "a mano": el programa toma la matriz de la barra rígida y elimina el
> grado de libertad liberado por **condensación estática**. Ver el
> [capítulo 6](06-fundamentos-teoricos.md#articulaciones).
...
2. **Construir Sección:** formas paramétricas, huecas (cajón, tubo, doble T, T, U, C) o macizas
   (rectangular, circular, T, L). El programa calcula las propiedades a partir de la geometría.
...
| **Resorte** | nada rígidamente: aporta rigidez kx, kz, kθ | proporcionales al desplazamiento |
...
**Carga** (tecla `L`) aplica cargas del caso de carga activo. Los tipos:
...
- **Distribuida.** Con un valor en cada extremo (**qI**, **qJ**), así que puede ser uniforme o
  trapezoidal. La dirección puede ser global (Z) o perpendicular a la barra, con un ángulo α
  adicional. Una carga sobre un tramo de la barra se define desde la tabla de cargas.
...
| **N** | Esfuerzo axial (positivo = tracción) |
...
- Para **Tensiones**, la **medida**: aprovechamiento σ/fy, Von Mises, tensión normal σ o
  tangencial τ.
